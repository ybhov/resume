...
\begin{tabularx}{\linewidth}{l|>{\raggedright\arraybackslash}X}
  ...
  \begin{itemize}[leftmargin=*]
    ...
    % \item Led migration and upgrade of a multi-million dollar application stack, comprising 70+ applications, from on-premises data centers to multiple AWS regions, leveraging Docker, Bitbucket Pipelines, and AWS CDK
...
% JavaScript (NodeJS, ExpressJS, Bun, VueJS) $\cdot$ SQL (MySQL, Postgres) $\cdot$ NoSQL DBs (DynamoDB, MongoDB, CouchDB) $\cdot$ Python ( FastAPI, Flask) $\cdot$ PHP (Laravel, Koseven) $\cdot$ IaC (AWS CDK) $\cdot$ CI/CD (CircleCI, Bitbucket Pipelines, GoCD) $\cdot$ RabbitMQ $\cdot$ Redis $\cdot$ Docker $\cdot$ Git 
